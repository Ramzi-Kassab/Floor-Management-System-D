...
class BitTypeListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = BitType.objects.select_related('size').filter(is_active=True)
        ...
        return queryset.order_by('category', 'series', 'smi_name')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = 'Bit Types'
        context['categories'] = BitType.Category.choices
        context['series_list'] = BitType.objects.values_list(
            'series', flat=True
        ).distinct().exclude(series='').order_by('series')
        return context
...
class BitTypeDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = f'Bit Type: {self.object.display_name}'
        # Get drill bits of this type
        context['drill_bits'] = DrillBit.objects.filter(
            product_type=self.object
        ).select_related('bit_location', 'customer')[:20]
        context['drill_bit_count'] = DrillBit.objects.filter(
            product_type=self.object
        ).count()
        return context
...
class BitTypeCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = [
        'category', 'size', 'smi_name', 'hdbs_name', 'series',
    ...
    def form_valid(self, form):
        # Auto-populate legacy code field
        if not form.instance.code:
            form.instance.code = form.instance.smi_name or form.instance.hdbs_name
        return super().form_valid(form)
...
class BitSizeListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return BitSize.objects.filter(is_active=True).order_by('size_decimal')
...
class BitSizeCreateView(LoginRequiredMixin, CreateView):
    ...
    model = BitSize
    template_name = "workorders/bitsize_form.html"
    fields = ['code', 'size_decimal', 'size_display', 'size_inches', 'is_active']
    success_url = reverse_lazy('workorders:bitsize_list')
...
class BitSizeUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = f'Edit: {self.object.size_display}'
        context['is_create'] = False
        return context
...
class LocationListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Location.objects.select_related('rig').filter(is_active=True)
...
class LocationDetailView(LoginRequiredMixin, DetailView):
    ...
    model = Location
    template_name = "workorders/location_detail.html"
    context_object_name = "location"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = f'Location: {self.object.name}'
        # Get drill bits at this location
        context['drill_bits'] = DrillBit.objects.filter(
            bit_location=self.object
        ).select_related('product_type', 'customer')[:20]
        context['drill_bit_count'] = DrillBit.objects.filter(
            bit_location=self.object
        ).count()
        return context
...
class LocationCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = ['code', 'name', 'location_type', 'rig', 'address', 'is_active']
