...
class DRSSRequestForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "drss_number": forms.TextInput(attrs={"class": TAILWIND_INPUT, "placeholder": "DRSS Number from ARAMCO"}),
            "external_reference": forms.TextInput(
                attrs={"class": TAILWIND_INPUT, "placeholder": "Additional reference (optional)"}
            ),
            ...
            "customer_notes": forms.Textarea(
                attrs={"class": TAILWIND_TEXTAREA, "rows": 3, "placeholder": "Notes from customer/ARAMCO"}
            ),
            "internal_notes": forms.Textarea(
                attrs={"class": TAILWIND_TEXTAREA, "rows": 3, "placeholder": "Internal notes (not visible to customer)"}
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["customer"].queryset = Customer.objects.filter(is_active=True, is_aramco=True).order_by("name")
    ...
    def clean(self):
        ...
        if requested_date and required_date:
            if required_date < requested_date:
                raise forms.ValidationError("Required date cannot be before requested date.")
...
class DRSSRequestLineForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "bit_type": forms.TextInput(attrs={"class": TAILWIND_INPUT, "placeholder": "FC / RC"}),
            "bit_size": forms.NumberInput(attrs={"class": TAILWIND_INPUT, "step": "0.001", "placeholder": "Size (inches)"}),
            "design_code": forms.TextInput(attrs={"class": TAILWIND_INPUT, "placeholder": "Design code"}),
            "quantity": forms.NumberInput(attrs={"class": TAILWIND_INPUT, "min": 1}),
            "iadc_code": forms.TextInput(attrs={"class": TAILWIND_INPUT, "placeholder": "IADC code"}),
            "formation": forms.TextInput(attrs={"class": TAILWIND_INPUT, "placeholder": "Formation type"}),
            "depth_from": forms.NumberInput(attrs={"class": TAILWIND_INPUT, "placeholder": "From (ft)"}),
            "depth_to": forms.NumberInput(attrs={"class": TAILWIND_INPUT, "placeholder": "To (ft)"}),
            "status": forms.Select(attrs={"class": TAILWIND_SELECT}),
            "fulfillment_option": forms.Select(attrs={"class": TAILWIND_SELECT}),
            "fulfillment_notes": forms.Textarea(
                attrs={"class": TAILWIND_TEXTAREA, "rows": 2, "placeholder": "Fulfillment notes"}
            ),
        }
...
DRSSRequestLineFormSet = inlineformset_factory(
    DRSSRequest, DRSSRequestLine, form=DRSSRequestLineForm, extra=1, can_delete=True
)
...
class DRSSEvaluationForm(forms.ModelForm):
    ...
    class Meta:
        model = DRSSRequestLine
        fields = ["status", "fulfillment_option", "fulfillment_notes", "source_bit", "work_order"]
        widgets = {
            "status": forms.Select(attrs={"class": TAILWIND_SELECT}),
            "fulfillment_option": forms.Select(attrs={"class": TAILWIND_SELECT}),
            "fulfillment_notes": forms.Textarea(attrs={"class": TAILWIND_TEXTAREA, "rows": 3}),
            "source_bit": forms.Select(attrs={"class": TAILWIND_SELECT}),
