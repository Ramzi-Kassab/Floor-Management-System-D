...
from .forms import DRSSRequestForm, DRSSRequestLineForm, DRSSRequestLineFormSet, DRSSEvaluationForm
...
class DRSSListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = DRSSRequest.objects.select_related(
            'customer', 'rig', 'well', 'received_by', 'evaluated_by'
        ).prefetch_related('lines').annotate(
            total_lines=Count('lines'),
            fulfilled_lines=Count('lines', filter=Q(lines__status='FULFILLED'))
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context['stats'] = {
            ...
            'pending': DRSSRequest.objects.filter(
                status__in=['RECEIVED', 'EVALUATING']
            ).count(),
            'due_this_week': DRSSRequest.objects.filter(
                required_date__range=[today, week_end],
                status__in=['RECEIVED', 'EVALUATING', 'CONFIRMED']
            ).count(),
            'overdue': DRSSRequest.objects.filter(
                required_date__lt=today,
                status__in=['RECEIVED', 'EVALUATING', 'CONFIRMED', 'PARTIAL']
            ).count(),
        }
...
class DRSSDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        return DRSSRequest.objects.select_related(
            'customer', 'rig', 'well', 'received_by', 'evaluated_by'
        ).prefetch_related(
...
def evaluate_line(request, pk):
    ...
    return render(request, 'drss/line_evaluate.html', {
        'form': form,
        'line': line,
        'drss': drss,
        'page_title': f'Evaluate Line {line.line_number}'
    })
...
def add_line(request, drss_pk):
    ...
    return render(request, 'drss/line_form.html', {
        'form': form,
        'drss': drss,
        'page_title': f'Add Line to {drss.drss_number}'
    })
...
def delete_line(request, pk):
    ...
    return render(request, 'drss/line_confirm_delete.html', {
        'line': line,
        'drss': drss,
        'page_title': f'Delete Line {line.line_number}'
    })
...
def export_csv(request):
    ...
    writer.writerow([
        'DRSS Number', 'Customer', 'Rig', 'Well', 'Status', 'Priority',
        'Requested Date', 'Required Date', 'Lines', 'Fulfilled'
    ])

    queryset = DRSSRequest.objects.select_related(
        'customer', 'rig', 'well'
    ).prefetch_related('lines').order_by('-requested_date')
