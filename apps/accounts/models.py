...
class User(AbstractUser):
    ...
    employee_id = models.CharField(max_length=20, unique=True, null=True, blank=True, help_text="HR Employee ID")
    department = models.ForeignKey(
        "organization.Department", on_delete=models.SET_NULL, null=True, blank=True, related_name="users"
    )
    position = models.ForeignKey(
        "organization.Position", on_delete=models.SET_NULL, null=True, blank=True, related_name="users"
    )
    ...
    signature = models.ImageField(upload_to="signatures/", null=True, blank=True, help_text="Digital signature for approvals")
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(fields=["email"], condition=models.Q(email__gt=""), name="unique_non_empty_email")
        ]
    ...
    def get_permissions(self):
        ...
        return list(Permission.objects.filter(roles__in=self.roles.all()).values_list("code", flat=True).distinct())
...
class Role(models.Model):
    ...
    # Hierarchy
    level = models.IntegerField(default=1, help_text="Role hierarchy level (higher = more authority)")
...
class UserPreference(models.Model):
    ...
    favorite_modules = models.JSONField(default=list, help_text="List of pinned module codes")
