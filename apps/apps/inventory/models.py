...
# NOTE: InventoryStock model REMOVED (Feb 2026) — replaced by StockBalance + VariantStock.
# See StockBalance (multi-dimensional) and VariantStock (variant-level) below.
# Migration removes the inventory_stock table.


class VariantStock(models.Model):
    """
    Stock tracking for item variants.
    Stock tracking for item variants by location.
    """
...
# NOTE: InventoryTransaction model REMOVED (Feb 2026) — replaced by StockLedger.
# StockLedger is the immutable ledger for all stock movements (qty_delta based).
# Migration removes the inventory_transactions table.
...
# NOTE: inventory.BillOfMaterial and inventory.BOMLine REMOVED (Feb 2026) — dead code.
# The active BOM system is in technology.BOM / technology.BOMLine.
# Migration removes the bill_of_materials and inventory_bom_lines tables.
