...
class Procedure(models.Model):
    ...
    category = models.CharField(max_length=20, choices=Category.choices, default=Category.PRODUCTION)
    ...
    responsible_role = models.ForeignKey(
        "accounts.Role", on_delete=models.SET_NULL, null=True, blank=True, related_name="responsible_procedures"
    )
    reviewed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="reviewed_procedures"
    )
    approved_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="approved_procedures"
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="created_procedures"
    )
    ...
    def estimated_duration(self):
        ...
        return self.steps.aggregate(total=models.Sum("estimated_duration_minutes"))["total"] or 0
...
class ProcedureStep(models.Model):
    ...
    dependency_condition = models.JSONField(
        null=True, blank=True, help_text='Condition: {"field": "result", "operator": "equals", "value": "PASS"}'
    )

    # Timing
    estimated_duration_minutes = models.IntegerField(null=True, blank=True)
    time_limit_minutes = models.IntegerField(null=True, blank=True, help_text="Max time allowed")
    ...
    form_template = models.ForeignKey(
        "forms_engine.FormTemplate", on_delete=models.SET_NULL, null=True, blank=True, related_name="procedure_steps"
    )

    # Expected output for validation
    expected_output = models.JSONField(
        null=True, blank=True, help_text='Expected outcomes: {"torque_min": 40, "torque_max": 50}'
    )
...
class CheckpointType(models.Model):
    ...
    code = models.CharField(max_length=30, unique=True)
    name = models.CharField(max_length=100)
    input_type = models.CharField(max_length=30, help_text="OK_NOK, PASS_FAIL, YES_NO, MEASURE, SELECT, TEXT, NUMBER")
...
class StepCheckpoint(models.Model):
    ...
    step = models.ForeignKey(ProcedureStep, on_delete=models.CASCADE, related_name="checkpoints")
    ...
    options = models.JSONField(null=True, blank=True, help_text="For dropdown options")
...
class StepBranch(models.Model):
    ...
    step = models.ForeignKey(ProcedureStep, on_delete=models.CASCADE, related_name="branches")
    sequence = models.IntegerField(default=0)
    ...
    condition_operator = models.CharField(max_length=20, help_text="=, !=, >, <, IN, CONTAINS, IS_NULL")
...
class StepInput(models.Model):
    ...
    step = models.ForeignKey(ProcedureStep, on_delete=models.CASCADE, related_name="inputs")
...
class StepOutput(models.Model):
    ...
    step = models.ForeignKey(ProcedureStep, on_delete=models.CASCADE, related_name="outputs")
