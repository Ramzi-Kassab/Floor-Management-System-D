...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Get Halliburton customer for LSTK variants
        halliburton = Customer.objects.filter(name__icontains='Halliburton').first()
        if halliburton:
            self.stdout.write(f"Halliburton customer: {halliburton.name} (ID={halliburton.id})")
        else:
            self.stdout.write(self.style.WARNING("Halliburton customer not found - LSTK variants won't have customer set"))
            ...
            if not confirm:
                ...
                try:
                    with transaction.atomic():
                        ...
                        # Set item_number attribute from "New Stock" column (primary ERP item number)
                        new_stock_erp = row_dict.get('New Stock')
                        if new_stock_erp and 'item_number' in category_attributes:
                            cat_attr = category_attributes['item_number']
                            ItemAttributeValue.objects.update_or_create(
                                item=item,
                                attribute=cat_attr,
                                defaults={
                                    'text_value': str(new_stock_erp).strip(),
                                }
                            )
                            # Also set the model's item_number field
                            item.item_number = str(new_stock_erp).strip()
                            item.save()
                        ...
                        for excel_col, variant_code in self.VARIANT_COLUMN_MAP.items():
                            ...
                            if erp_item_number and str(erp_item_number).strip():
                                ...
                                # Determine if this is an LSTK variant (needs account and customer)
                                is_lstk = variant_code in self.LSTK_VARIANTS
                                ...
                                if existing_variant:
                                    ...
                                    if existing_variant.erp_item_no != erp_item_number:
                                        ...
                                        updated = True
                                    # Update account/customer for LSTK variants
                                    if is_lstk:
                                        if existing_variant.account != 'LSTK':
                                            existing_variant.account = 'LSTK'
                                            updated = True
                                        if halliburton and existing_variant.customer != halliburton:
                                            existing_variant.customer = halliburton
                                            updated = True
                                    if updated:
                                    ...
                                    try:
                                        ...
                                        # Set account and customer for LSTK variants
                                        if is_lstk:
                                            variant_data['account'] = 'LSTK'
                                            if halliburton:
                                                variant_data['customer'] = halliburton
                                        ItemVariant.objects.create(**variant_data)
                                        variants_created += 1
