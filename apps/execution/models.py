...
class ProcedureExecution(models.Model):
    ...
    procedure = models.ForeignKey("procedures.Procedure", on_delete=models.PROTECT, related_name="executions")
    work_order = models.ForeignKey("workorders.WorkOrder", on_delete=models.CASCADE, related_name="procedure_executions")

    # Version tracking (captures procedure version at execution time)
    procedure_version = models.CharField(max_length=10, blank=True)
    procedure_snapshot = models.JSONField(null=True, blank=True, help_text="Snapshot of procedure at execution time")

    status = models.CharField(max_length=20, choices=Status.choices, default=Status.PENDING)

    # Progress
    current_step = models.ForeignKey(
        "procedures.ProcedureStep", on_delete=models.SET_NULL, null=True, blank=True, related_name="current_executions"
    )
    ...
    started_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="started_executions"
    )
    completed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="completed_executions"
    )
...
class StepExecution(models.Model):
    ...
    executed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="executed_steps"
    )
...
class CheckpointResult(models.Model):
    ...
    # For measurements
    measured_value = models.DecimalField(max_digits=10, decimal_places=4, null=True, blank=True)
