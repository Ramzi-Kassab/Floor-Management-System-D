...
- **Mouse Wheel Scroll Replaces PageDown**: `click_dynamic_locator` grid scrolling completely rewritten from keyboard PageDown to `page.mouse.wheel(0, 300)`. PageDown was unreliable because it depends on grid keyboard focus — clicking a grid cell enters edit mode, and Escape doesn't always exit it. Mouse wheel events work regardless of focus state. Positions mouse over `.fixedDataTableLayout_body` center (fallback: first `input.dyn-hyperlink` position), then scrolls in 300px increments with 500ms wait between each. Overshoot detection and scroll-back via `mouse.wheel(0, -300)` still works. Grid bottom detection via unchanged last visible value.
