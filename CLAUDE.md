...
- **New Workflow WF-7B (pk=92)**: Replaces WF-7 (Enter BOM Lines), WF-8 (Approve BOM), and WF-90 (recorded Edit BOM Version) as a single workflow in chain 10 at order 90. 45 steps total: Phase 1 (delete all), Phase 2 (add 8 BOM lines), Phase 3 (approve + activate).
- **Phase 1 — Bulk Delete Existing BOM Lines**: Steps 10-12. Select-all checkbox (locator 500: `d365_bom_select_all_checkbox`) → Delete button (locator 447: `DeleteBOMLine_label`) → Confirm Yes (locator 448). All `continue_on_error=True`.
- **Phase 2 — Add BOM Lines 1-8**: Steps 20-96 (4 steps per line × 8). Pattern: New BOM Line (click, locator 449) → Fill Item Number (fill, combobox, Enter, locator 450, template `{{BOM_LINE_N_ITEM}}`) → Confirm Item (click, locator 451) → Fill Quantity (fill, standard_input, Tab, locator 452, template `{{BOM_LINE_N_QTY}}`). All `continue_on_error=True` so empty template variables (lines beyond actual BOM count) skip gracefully.
- **Phase 3 — Approve & Activate**: Steps 100-109. BOM Action Pane Tab → Approve BOM → OK → Close BOM Table → Wait 2s → BOM Version Tab → Version Approve → OK → Activate → Close. These steps have `continue_on_error=False`.
- **Chain 10 Updated**: Old WF-7 (pk=85, order 10100), WF-8 (pk=86, order 10110), WF-90 (pk=90, order 10150), WF-91 (pk=91, order 10085) all deactivated. WF-7B inserted at order 90. 12 active links with clean 10-spacing (10-120).
- **D365 BOM Grid Locator Challenges (Known Issues)**:
  - `InventTable_ItemId` inputs exist in 3 contexts: lookup result row (`dyn-field`), grid cells (`dyn-hyperlink` — navigates on click!), and product detail page (`textbox` class, `required="true"`). Locator 451 uses: `//input[contains(@id, "InventTable_ItemId") and @aria-label="Item number" and @readonly and not(contains(@class, "dyn-hyperlink")) and not(contains(@class, "textbox"))]`
  - D365 FixedDataTable renders duplicate header cells (one in `fixedDataTableLayout_header`, one shadow copy). Select-all checkbox locator (500) scoped to: `//*[contains(@id, "BOMTable")]//*[contains(@class, "fixedDataTableLayout_header")]//*[@role="checkbox" and contains(@class, "dyn-markingColumn")]`
  - BOM-specific buttons use `DeleteBOMLine`/`NewBomLine` IDs (NOT `SystemDefinedDeleteButton`/`SystemDefinedNewButton`).
  - Quantity locator 452 (`name=BOM_BOMQty`) has 0 successes / 6 failures — needs fix to `contains(@id)` pattern.
  - Clicking any `dyn-hyperlink` field in the BOM grid navigates to a detail page. Configuration field (`InventoryDimensionsGrid_configId`) is particularly dangerous — has `dyn-hyperlink` class and `aria-haspopup="dialog"`.
- **WF-7B Fill Approach — Work In Progress**: The `combobox` fill mode (click → Ctrl+A → Delete → type) sometimes clicks a hyperlink grid cell instead of the editable input. The keyboard-only `press_key` approach (type directly after New BOM Line auto-focuses item field) works in step-by-step mode but needs further testing in continuous mode. Current DB state uses the original `fill` + `click confirm` pattern for compatibility with the debug UI.
- **Debug UI Run Icon Requirement**: The per-step play triangle in `chain_detail.html` requires: `debugState.active === true` AND `debugState.status === 'paused'` AND `debugState._runningLinkOrder === seg.link_order`. Icons only appear after the first pause (e.g., failed step or step-by-step mode). Green play-circle on segment header shows for any paused state. Steps are cached from page load — must refresh (F5) after DB step changes to load new PKs.
- **New Locators Created**: PK 500 (`d365_bom_select_all_checkbox`), 501 (`d365_bom_config_field` — unused, clicking navigates), 502 (`d365_bom_config_lookup_btn`), 503 (`d365_bom_config_variant_select`), 504 (`d365_bom_item_lookup_first_row`).
