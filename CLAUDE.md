...
- **Die Check Symbols**: 4 symbols: O (OK/No Indication), C (Cracked), H (Chipped), P (Porosity). O is exclusive with all other findings.
...
- **Auto-Generated Summary**: `autoRemark` computed property generates a multi-line text summary of the die check (stage, WO, serial, result, all flagged cutters with findings/decisions/remarks). Copy button copies to clipboard for sharing as the conclusion without sharing the full table.
...
- **Photo Module: Download, Copy, Download All** (`templates/components/photo_module.html`): Three new buttons on gallery thumbnails hover overlay: Download (saves file with display_name), Copy to clipboard (converts to PNG via canvas for Clipboard API compatibility), Download All (sequential download of all photos). All pages using photo_module.html (receiving inspection, cutter evaluation, pre-repair evaluation, die check, drill bit detail, work order detail) automatically get these features.
